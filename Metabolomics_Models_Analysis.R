...
#categorical: 170 (M/F), 172 (White/Black), 173 (T/F), 336 (T/F), 335 (T/F)
...
#fit models on training dataset (hdnom(glmnet), 10-fold CV, alpha optimisation)
...
#individual metabolite associations with age+sex or PCP-HF adjusted COX-PH models
...
#empty bars at the end of each group
to_add <- data.frame(matrix(NA, 2*nlevels(circ_agesex$Class), ncol(circ_agesex))) #NA values at end of each group
colnames(to_add) <- colnames(circ_agesex) 
to_add$Class <- rep(levels(circ_agesex$Class), each=2)    #empty bar between groups
circ_agesex <- rbind(circ_agesex, to_add)   
circ_agesex <- circ_agesex %>% arrange(Class)   
...
#subtract 0.5 because the labels must have the angle of the center of the bars. Not extreme right(1) or extreme left (0)
label_data$hjust <- ifelse( angle < -90, 1, 0)    # horizontal location of the label, relative to the bar
#in left part of the plot, labels currently at < -90
label_data$angle <- ifelse(angle < -90, angle+180, angle)   # flip angle of label by 180 to make them readable
label_data$color <- ifelse(label_data$p_adj < 0.01, "black", "darkgrey")   #if not significant, biomarker will appear grey
...
base_data$title[8] <- base_data$title[8]-5 #subtract half of the additional emtpy rows here
title_angle <- base_data$title*360/number_of_bar    #calculate the angle of label for each bar
...
circ_p_agesex <- ggplot(circ_agesex, aes(x=as.factor(id), y=hr-1, fill=hr-1)) +
  ...
  scale_fill_gradient2(low="blue", mid="white", high="red", midpoint=0, limits=c(-0.35, 0.35)) +   #Max value of bar is between 0.22 & 0.23, min is between 1.9 & 0.2
...
#empty bars at the end of each group
to_add <- data.frame(matrix(NA, 2*nlevels(circ_PCP$Class), ncol(circ_PCP)))   #NA values at end of each group
colnames(to_add) <- colnames(circ_PCP)   
to_add$Class <- rep(levels(circ_PCP$Class), each=2)    #empty bar between groups
circ_PCP <- rbind(circ_PCP, to_add)   
circ_PCP <- circ_PCP %>% arrange(Class)   
#add some empty bars at the very end
...
title_angle <- base_data$title*360/number_of_bar    #calculate the angle of label for each category
base_data$title_angle <- ifelse(90<title_angle & title_angle<270, title_angle - 180, title_angle)   #flip angle by 180 degrees if title angle is between 90 and 270 degrees
#remove final 10 rows
base_data$end[8] <- base_data$end[8]-10
...
for (var in categorical_vars) {
  if (var == "Sex") {
    #specific setting for "Sex" variable (include two lines for F and M)
    for (value in c("F", "M")) {
    ...
    #specific setting for "Ethnic.background...Instance.0" variable (include two lines for B and W)
    for (value in c("B", "W")) {
      count_PCP <- sum(BL_characteristics_PCP[[var]] == value, na.rm = TRUE)
      count_Train <- sum(BL_characteristics_Train[[var]] == value, na.rm = TRUE)
      count_Test <- sum(BL_characteristics_Test[[var]] == value, na.rm = TRUE)
...
#adjustment for multiple training
...
#NRI, available packages were not clearly documented so manual implementation
...
#rel IDI divides by ref model Yates slope
#here calculated on the calibration dataset with exclusion of pre-10y non-HF censoring and predicted risk at 10y
...
#figure 3: Incidence and survival curves
#incidence
...
rownames(coefs_agesex)
rownames(coefs_agesex_met)
rownames(coefs_PCP)
rownames(coefs_PCP_met)
...
softPower = 30 # picked after inspection of plots
#adjacency matrix
...
color_palette <- c("#ffd502","#ff0100","#03ce03","#00999a", "blue", "#7309ae") #length of groups
lightness_factor <- 0.15
node_colors <- rep("gray", vcount(network))  #initialize nodes
...
#CV performance within validation cohort
#partition validation split
partition_indices <- list()
for (i in 1:10) {
  partition_indices[[i]] <- createDataPartition(PCP$HF, p = 0.9, list = FALSE, times = 1)
}
#CV loop
