...
#fetch data
Complete <- read.delim("Final dataframes/Before_exclusion_300k_final.tsv", sep = '\t')
Complete_100k <- read.delim("Final dataframes/Before_exclusion.tsv", sep = '\t')

table(colnames(Complete_100k) %in% colnames(Complete))
colnames(Complete)[!(colnames(Complete) %in% colnames(Complete_100k))]
colnames(Complete_100k)[!(colnames(Complete_100k) %in% colnames(Complete))]
names(Complete)
names(Complete_100k)
...
results_PCP <- data.frame(variable = character(),
                          ...
                          stringsAsFactors = FALSE)
for (i in 1:168) {
  # Create a Cox regression model for the current column
  model <- coxph(Surv(followup_HF, HF) ~ PCP[,i] 
                 + Age + Sex,
                 data = PCP)
  
  # Extract the hazard ratio, confidence intervals, and p-values
  hr <- exp(coef(model)[1])
...
result_df <- data.frame()  # Create an empty dataframe to store the results
